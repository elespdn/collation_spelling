...
try:
    os.makedirs('data/example2/process')
except OSError:
    pass

# put together the texts of all the witnesses
with open('data/example2/W1.txt') as W1, \
     open('data/example2/W2.txt') as W2, \
     open('data/example2/W3.txt') as W3, \
     open('data/example2/W4.txt') as W4, \
     open('data/example2/process/p1_all_texts.txt', 'w', encoding='utf-8') as all_texts:
    all_texts.write(W1.read() + ' ' + W2.read() + ' ' + W3.read() + ' ' + W4.read())

# each word a new line, in alphabetical order: sorted()
infile = open('data/example2/process/p1_all_texts.txt').read()
outfile = open('data/example2/process/p2_all_words.txt', 'w', encoding='utf-8')
words = infile.split()
for word in sorted(words):
    outfile.write(word + '\n')

# delete duplicates
infile = 'data/example2/process/p2_all_words.txt'
lines_seen = set() # holds lines already seen
outfile = open('data/example2/process/p3_single_words.txt', 'w', encoding='utf-8')
...
infile = 'data/example2/process/p4_single_words_analyzed.txt'
...
infile = open('data/example2/process/p5_single_words_analyzed_clean.txt', 'r')
outfile = open('pos_lemma_example2.csv', 'w', encoding='utf-8')
for aline in infile:
    values = aline.split()
    original = values[0]
    pos = values[1]
